...
/*
 ⚠️注意：前方高能，很多坑！!!
 思路就是把备注点放在 positions 数组里。每次添加新备注点都 push positions 数组，positions 通过位置（lan、lng）跟 Marker 点关联。
 不管是添加、修改、删除 Marker 的操作，都遍历更新一次 positions。
*/ 
...
function initialize() {
    ...
    google.maps.event.addListener(infowindow, 'domready', function() {
        // console.log('domready happen');

        $(".gm-style-iw").next("div").hide(); // 隐藏infowindow关闭按钮

        // var cc_value = $("#waypointContent").val();

        var dom_ready_lat = infowindow.marker.position.lat();
        var dom_ready_lng = infowindow.marker.position.lng();

        for (var i = 0; i < positions.length; i++) {
            if (positions[i]["lat"] === dom_ready_lat && positions[i]["lng"] === dom_ready_lng) {
        ...
        $("#waypointDelete").click(function() {
            ...
            var nlat = infowindow.marker.position.lat();
            var nlng = infowindow.marker.position.lng();
            // content = infowindow.marker.content;
            infowindow.marker.content = "";
    ...
    menu.addItem('添加备注', function(map, latLng) {
        ...
        google.maps.event.addListener(marker, 'click', function() {
            ...
            for (var i = 0; i < positions.length; i++) {
                if (positions[i]["lat"] === marker_lat && positions[i]["lng"] === marker_lng) {
                    $("#waypointContent").val(positions[i]['content']);
                }
            }      
        });
        ...
        google.maps.event.addListener(marker, 'dragstart', function(event) {
            dragstartlat = event.latLng.lat(); // 获取 dragstart 时 lat
            dragstartlng = event.latLng.lng(); // 获取 dragstart 时 lng
            return;
        });

        // 拖拽 marker 到的新位置
        google.maps.event.addListener(marker, 'dragend', function(event) {
            // 用新位置替换 positions 里的原位置
            for (var i = 0; i < positions.length; i++) {
                if (positions[i]['lat'] === dragstartlat) {
                    positions[i]['lat'] = event.latLng.lat();
                    positions[i]['lng'] = event.latLng.lng();
                } else {
                    console.log('no');
                }
            }
